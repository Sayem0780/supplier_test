...
class CPrint extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final String calanNo = cart.register[0].code;
    ...
    return WillPopScope(
      ...
      child: Scaffold(
        ...
        body: PdfPreview(
          build: (formate) => createPDF(cart.selectedItems.values.toList(), 15,name,phone,address,prpo,quantity,calanNo,ShopName,Address,Phone),
        ),
      ),
    );
  }
  ...
  Future<Uint8List> createPDF(
      List<CartItem> items, int itemsPerPage,name,phone,address,prpo,total, String calanNo,ShopName,Address,Phone) async {
    String currentDate = DateFormat('dd/MM/yyyy').format(DateTime.now());
    var calanNumber = calanNo.toString();
    final pdf = pw.Document();
    final itemData = items;
    // final itemData = items.values.map((item) => [
    //   item.id,
    //   item.title,
            ...
            for (var j = 0; j < itemsPerPage; j++) {
              ...
              rows.add(pw.TableRow(
                children: [
                  ...
                  pw.Padding(
                    padding: const pw.EdgeInsets.all(4),
                    child: pw.Center(child: pw.Text(item.uom)),
                  ),
                ],
              ));
            }
                                    ...
                                    children: [
                                      ...
                                      pw.TableRow(
                                          ...
                                          children: [
                                            ...
                                            pw.Padding(
                                              padding: const pw.EdgeInsets.all(4),
                                              child: pw.Center(child:pw.Text(total.toString())),
                                            ),
                                          ]
                                      ),
                                    ],
            ...
            return pw.Stack(
                children: [
                  pw.Center(
                    child:   pw.Transform.rotate(
                      angle: 3.14159 / .5643,
                      child:    pw.Text('$ShopName',style: pw.TextStyle(color: PdfColors.grey,fontSize: 76,
                        fontWeight: pw.FontWeight.normal,fontStyle: pw.FontStyle.italic,background: const pw.BoxDecoration(color: PdfColors.white),)),
                    ),
                  ),
                  pw.Column(
                      mainAxisAlignment: pw.MainAxisAlignment.spaceBetween,
